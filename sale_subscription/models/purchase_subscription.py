...
class SaleSubscription(models.Model):
    ...
    recurring_invoice_line_ids = fields.One2many(
        'sale.subscription.line', 's_subscription_id', string='Invoice Lines', copy=True)
    ...
    tag_ids = fields.Many2many('sale.subscription.tag', 'sale_subscription_tag_rel', 'tag_id', string='Tags')
    description = fields.Text()
    user_id = fields.Many2one('res.users', string='Sales Rep')
    invoice_ids = fields.One2many('account.move', 'subscription_id_so')
    invoice_count = fields.Integer(compute='_compute_invoice_count')
    partner_id = fields.Many2one('res.partner', string="Customer", domain="[('customer_rank', '=', 1)]")
    code = fields.Char(string='Reference', index=True, default=lambda self: self.env[
                       'ir.sequence'].next_by_code('sale.subscription') or 'New')
    user_id = fields.Many2one('res.users', string="Sales Rep")
    ...
    def unlink(self):
        if not self:
            return True
        print("@@@@@@@@@@@@@@@@@@@@@@2",self.recurring_invoice_line_ids) 
        for val in self.recurring_invoice_line_ids:
            val.unlink()
    ...
    def _track_subtype(self, init_values):
        ...
        if 'state' in init_values:
            return 'sale_subscription.subtype_state_change_sale'
    ...
    def get_info_partner(self):
        ...
        for sale in self:
            ...
            payment_term = False
            if sale.company_id:
                currency = sale.company_id.currency_id
            if sale.partner_id:
                if sale.partner_id.currency_id:
                    currency = sale.partner_id.currency_id
                elif sale.partner_id.property_product_pricelist:
                    currency = sale.partner_id.property_product_pricelist.currency_id
                if sale.partner_id.property_supplier_payment_term_id:
                    payment_term = sale.partner_id.property_supplier_payment_term_id
    ...
    @api.model
    def cron_sale_subscription(self):
    ...
    def _prepare_invoice_data(self):
        ...
        """ Get the sale journal of the company """
        ...
        print("self.partner_id.property_account_payable_id.id@@@@@@@@@@@@@@@@@@",self.partner_id.property_account_payable_id)
        return {
            # 'analytic_account_id': self.partner_id.property_account_payable_id.id,
            'type': 'in_invoice',
    ...
    def _prepare_invoice_line(self, line, fiscal_position):
        ...
        tax = line.product_id.supplier_taxes_id.filtered(
            lambda r: r.company_id == line.s_subscription_id.company_id)
        ...
        for company_id in set(data['company_id'][0] for data in sub_data):
            ...
            for sub in subs:
                try:
                    ...
                    print("invoices[-1]@@@@@@@@@@@@@@@@@@@@",invoices[-1])
                    print("sub@@@@@@@@@@@@@@@@@@@@",sub.invoice_ids)
                    valuses = invoices[-1].id
                    sub.invoice_ids = [(4, valuses)]
...
class SaleSubscriptionLine(models.Model):
    ...
    product_id = fields.Many2one('product.product', string='Product',
                                 domain="[('recurring_invoice_so','=',True)]", required=True)
    s_subscription_id = fields.Many2one(
    ...
    def onchange_product_id(self):
        ...
        domain = {}
        contract = self.s_subscription_id
